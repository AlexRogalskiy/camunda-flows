...
export const getOpenTaskDetails = () : AppPromiseThunkAction => {
    return (dispatch: AppThunkDispatch): Promise<OperationResponse> => {
        let requestUrl = BACKEND_API  + "/openTaskDetails";
        return axios.post(requestUrl)
            .catch(err => console.log("getOpenTaskDetails:error:" + err))
            .then(response => {
                if(response) {
                    let operationResponse:OperationResponse= response.data;
                    if(operationResponse !== null){
                        dispatch(setOpenTaskDetails(operationResponse))
                        return operationResponse;
                    }
                }
            }) as Promise<OperationResponse>;
...
export const setOpenTaskDetails = (operationResponse:OperationResponse) : AnyAction => {
    return {
        type: SessionStartActionTypes.SET_OPEN_TASK_DETAILS,
        openTaskList: operationResponse.responseObject
    };
};

export const getQuotaInfo = () : AppPromiseThunkAction => {
    return (dispatch: AppThunkDispatch): Promise<OperationResponse> => {
        let requestUrl = BACKEND_API  + "/quotaInfo";
        return axios.post(requestUrl)
            .catch(err => console.log("getQuotaInfo:" + err))
            .then(response => {
                if(response) {
                    let operationResponse:OperationResponse= response.data;
                    if(operationResponse !== null){
                        dispatch(setQuotaInfo(operationResponse))
                        return operationResponse;
                    }
                }
            }) as Promise<OperationResponse>;
...
export const setQuotaInfo = (operationResponse:OperationResponse) : AnyAction => {
    return {
        type: SessionStartActionTypes.SET_QUOTA_INFO,
        quotaInfo: operationResponse.responseObject
    };
};